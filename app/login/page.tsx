...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        router.refresh(); // Refresh state to let middleware/layout pick up the session
      ...
      <div className="w-full max-w-md bg-white border border-zinc-100 rounded-3xl p-8 shadow-xl relative z-10 space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 font-semibold mt-1">Sign in to your Delhi Public School account</p>
          </div>
        </div>
        ...
        {error && (
          <div className="bg-red-50 border border-red-100 text-red-600 rounded-2xl p-4 flex items-start gap-3 animate-shake">
            ...
            <div className="min-w-0">
              <h4 className="text-xs font-bold leading-none">Authentication Error</h4>
        ...
        {success && (
          <div className="bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div className="min-w-0">
              <h4 className="text-xs font-bold leading-none">Access Authorized</h4>
              <p className="text-[10px] font-semibold leading-relaxed mt-1">Loading dashboard overview...</p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center px-1">
              <label className="text-[9px] font-black uppercase text-zinc-400 tracking-wider">Password</label>
              <a href="#" className="text-[10px] text-zinc-400 hover:text-brand-orange font-bold">Forgot?</a>
            </div>
            <div className="relative">
              <input
                type="password"
        ...
        <div className="pt-6 border-t border-zinc-100 text-center">
          <p className="text-xs font-medium text-zinc-400">
            New to VedaAI academic infrastructure?{' '}
            <Link 
              href="/signup" 
